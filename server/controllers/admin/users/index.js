    ...
    try {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            let monthCount = await Users.aggregate([
                { $match: { createdAt: { $gte: new Date(curr), $lt: new Date(d) }, usertype: 'user' } },
                {
                    $group: {
                        _id: "",
                        users: { $sum: 1 },
...
router.get("/top/emails", authMiddleware, async (req, res) => {
    try {
        ...
        let top10 = await Users.find({ usertype: "user" }, { password: 0, userverified: 0, userverifytoken: 0, passwordresettoken: 0, createdAt: 0 }).sort({ 'count.email': -1 }).limit(10)
